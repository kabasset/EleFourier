...
struct LocalZernikeSeries {
  ...
  void ansiSeq(double* dst, long count = JCount) { // FIXME could be templated by some program-wise count
    const auto array = ansiSeq<JCount>();
    std::copy_n(array.begin(), count, dst); // FIXME throw if count > JCount
  }
};

/**
 * @brief Generate Zernike polynomials for each ANSI index over a square domain.
 * @param side The domain side
 * @param count The number of indices
 * @details
 * The axes are ordered as (lambda, u, v) for performance:
 * for each point, indices are contiguous in memory.
 */
Fits::VecRaster<double, 3> ansiCube(long side, long count) {
  const Fits::Position<3> shape = {count, side, side};
  const double radius = 0.5 * side;
  Fits::VecRaster<double, 3> zernike(shape);
  for (long v = 0; v < side; ++v) {
    for (long u = 0; u < side; ++u) {
      LocalZernikeSeries series(u, v, radius, 0); // Cannot use NaN for DFTs
