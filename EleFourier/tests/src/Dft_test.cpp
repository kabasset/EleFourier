...
void checkFftwMalloc(const Fits::Position<2>& shape, long count) {
  auto raster = initFftwBuffer<T>(shape, count, nullptr);
...
BOOST_AUTO_TEST_CASE(fftw_r2c_malloc_test) {
  ...
  long count = 5;
  auto in = initFftwBuffer<double>(shape, count, nullptr);
  auto out = initFftwBuffer<std::complex<double>>(shape, count, nullptr);
...
BOOST_AUTO_TEST_CASE(init_test) {
  ...
  checkInit<RealDft::Inverse>(shape, count);
  checkInit<ComplexDft>(shape, count);
  checkInit<ComplexDft::Inverse>(shape, count);
}

template <typename TPlan>
void checkInverse(const Fits::Position<2>& shape, long count) {
  DftPlan<typename TPlan::Type> plan(shape, count);
...
BOOST_AUTO_TEST_CASE(inverse_test) {
  const Fits::Position<2> shape {4, 3};
  const long count = 10;
  RealDft::Inverse(shape, count);
  checkInverse<RealDft::Inverse>(shape, count);
  ComplexDft::Inverse(shape, count);
  checkInverse<ComplexDft::Inverse>(shape, count);
}
...
BOOST_AUTO_TEST_CASE(real_inverse_composition_test) {
  checkComposition<RealDft, RealDft::Inverse>();
  checkComposition<RealDft::Inverse, RealDft>();
}

// BOOST_AUTO_TEST_CASE(real_complex_composition_test) {
//   checkComposition<RealDft, HermitianComplexDft>();
//   checkComposition<HermitianComplexDft::Inverse, RealDft::Inverse>();
// }

BOOST_AUTO_TEST_CASE(complex_inverse_composition_test) {
  checkComposition<ComplexDft, ComplexDft::Inverse>();
  checkComposition<ComplexDft::Inverse, ComplexDft>();
}

BOOST_AUTO_TEST_CASE(complex_complex_composition_test) {
  checkComposition<ComplexDft, ComplexDft>();
  checkComposition<ComplexDft::Inverse, ComplexDft::Inverse>();
}
