...
from PyFFTPlayground.DFT import DFT, create_complex_plan, create_real_plan
from PyFFTPlayground.Timer import Timer


class BranchDFTs:
    def __init__(self, mask_side, psf_side, logger):
        self.pupil_to_psf = create_complex_plan(mask_side * mask_side)
        self.psf_to_mtf = create_real_plan(mask_side * mask_side)
        self.mtf_to_broadband = create_real_plan(psf_side * psf_side)
...
def mainMethod(args):
    ...
    if params > 1:
        ...
        return Exit.Code["NOK"]

    logger.info("# Initialize FFTW plans sequentially...")
    main_chrono.start()
    plans = [BranchDFTs(mask_side, broadband_side, logger) for p in range(params)]
    main_chrono.stop()

    for plan in plans:
        # Shortcuts
        dft_pupil_to_psf = plan.pupil_to_psf.transform
        psf_to_mtf = plan.psf_to_mtf.transform
        mtf_to_broadband = plan.mtf_to_broadband.inverse
        chrono = plan.chrono

        # Store some of mono DFT
        ...
        for l in range(lambdas):
            ...
            input = np.random.randn(mask_side * mask_side)
            # Compute DFT
            ...
            psf_mono = dft_pupil_to_psf.__call__(input_array=input)
            chrono.stop()
            ...
            chrono.start()
            mtf = psf_to_mtf.__call__(input_array=abs_psf_mono)
            chrono.stop()
        ...
        broadband_size = mtf_to_broadband.input_array.size
        mtf_broad_sum = mtf_broad_sum[:broadband_size]

        # Then compute the MTF broadband (inverse)
        logger.info("# Compute DFT broadband...")
        chrono.start()
        broadband = mtf_to_broadband.__call__(input_array=mtf_broad_sum)
        chrono.stop()
